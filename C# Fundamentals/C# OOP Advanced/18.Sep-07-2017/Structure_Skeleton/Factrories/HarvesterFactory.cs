...
public class HarvesterFactory : IHarvesterFactory
{
    public IHarvester GenerateHarvester(IList<string> args)
    {
        ...
        Type classInstance = Assembly.GetExecutingAssembly().GetTypes().FirstOrDefault(t => t.Name == type + "Harvester");

        var ctors = classInstance.GetConstructors(BindingFlags.Public | BindingFlags.Instance);
