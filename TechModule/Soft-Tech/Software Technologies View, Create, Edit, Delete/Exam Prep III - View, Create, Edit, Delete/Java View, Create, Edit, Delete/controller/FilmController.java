...
public class FilmController {
	...
	public String createProcess(Model model, FilmBindingModel filmBindingModel) {
		...
		if(filmBindingModel.getName().equals("") || filmBindingModel.getGenre().equals("")
			...
			model.addAttribute("view", "film/create"); // Redirect-ваме към film/create
			return "base-layout";
		}
		...
		filmRepository.saveAndFlush(film); // Запазваме в Базата
		return "redirect:/";
	}

	@GetMapping("/edit/{id}")
	public String edit(Model model, @PathVariable int id) {
		Film film = filmRepository.findOne(id); // Намираме конкретния Film по ID
	...
	public String editProcess(Model model, @PathVariable int id, FilmBindingModel filmBindingModel) {
		if(filmBindingModel.getName().equals("") || filmBindingModel.getGenre().equals("")
				|| filmBindingModel.getDirector().equals("") || filmBindingModel.getYear() == null){
			model.addAttribute("film", filmBindingModel);
			model.addAttribute("view", "film/edit"); // Redirect-ваме към film/create
			return "redirect:/";
		}

		Film film = filmRepository.findOne(id); // Намираме конкретния Task по ID
